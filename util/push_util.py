...
def push_results(exec_results, config: PushConfig, force_push: bool = False,
    ...
    :param title: 自定义推送标题，为 None 时用各渠道默认标题
    :param note: 追加到正文末尾的补充说明（如失败提示），为 None 时不追加
    """
    ...
    push_to_server_chan(exec_results, config, title=title, note=note)
    push_to_push_plus(exec_results, config, title=title, note=note)
    push_to_wechat_webhook(exec_results, config, title=title, note=note)
...
def push_to_server_chan(exec_results, config: PushConfig, title: str = None, note: str = None):
    """Server酱推送"""
...
def push_to_push_plus(exec_results, config: PushConfig, title: str = None, note: str = None):
    """PushPlus推送（HTML）"""
    ...
    if note:
        content = f"{content}<br>{note}"
    html = f'<div>{content}</div>'

    title = title or f"{format_now_bj()} 刷步通知"
    print(f"[推送] 发送PushPlus...")
    push_plus(config.push_plus_token, title, html)


def push_to_wechat_webhook(exec_results, config: PushConfig, title: str = None, note: str = None):
    """企业微信推送（Markdown V2）"""
