...
## 功能特点
- **定时与重试**：cron-job.org 早窗（10:00~10:20 每 5 分钟 + 11:00 保底）、晚窗（19:00~19:20 每 5 分钟 + 20:00 保底）触发，也可手动触发；单次运行内登录重试 3 次、步数提交重试 5 次（指数退避）；整窗失败则不写去重状态，由后续触发点自动补跑，**早晚各只成功刷一次**。
