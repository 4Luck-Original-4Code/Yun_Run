...
这是一个基于 GitHub Actions 自动运行的 Zepp 步数修改脚本，每天定时自动刷步；**仅支持单个账号**，适合个人使用。
...
```text
[cron-job.org]  早窗 10:00~10:10 / 11:00，晚窗 19:30~19:40 / 20:00（北京时间）
...
| 任务 | Cron 表达式 | 触发时间 |
|---|---|---|
| 早上主刷 | `0,10 10 * * *` | 10:00 / 10:10 |
| 早上保底 | `0 11 * * *` | 11:00 |
| 晚上主刷 | `30,40 19 * * *` | 19:30 / 19:40 |
| 晚上保底 | `0 20 * * *` | 20:00 |
...
- 若反复登录失败（达到最大重试次数），可能是账号需要验证：登录Zepp绑定第三方后，再手动触发一次刷步。
